...
class BankApp:
    def __init__(self, root):
        ...
        self.bank_lib = ctypes.CDLL(r"C:\Users\ashar\OneDrive\Desktop\School projects\bank-management-app-cash-flow-main\bank-management-app-cash-flow-main\build\bank.dll")
    ...
    def setup_library_functions(self):
        ...
        self.bank_lib.Bank_find_client.argtypes = [
    ctypes.POINTER(Bank),
    ctypes.c_char_p,
    ctypes.c_char_p,
    ctypes.c_int,
    ...
    def show_client_info(self):
        ...
        client_name = ctypes.create_string_buffer(256)
        ssn = ctypes.create_string_buffer(256)
        bank_account_number = ctypes.create_string_buffer(256)
        balance = ctypes.c_double()

        found = self.bank_lib.Bank_find_client(
            self.bank,
            account.encode(),
            client_name,
            256,
        ...
        if found:
            info = f"Name: {client_name.value.decode()}\n" \
               f"SSN: {ssn.value.decode()}\n" \
               f"Account: {bank_account_number.value.decode()}\n" \
               f"Balance: {balance.value:.2f}"
